...
- **Cosmic Omen** (level 6). **Reaction**, when a creature he can see within 30 ft is about to make a D20 Test: Weal **+1d6+2** or Woe **-1d6-2** (the flat +2 comes from Reader of Omens; the base feature is a plain 1d6). **Uses = Wis mod = 5/long rest** (base form: even roll = Weal, odd = Woe for the day; Door B removes that limit). Declare **before** the die is rolled in the base version; **Ursa took Reader of Omens, so he declares AFTER the die lands** (Read the Moment, below). A sim must resolve his Reaction with the roll already known.
  - **RESOLVED (DM, 2026-09-05): SELF-TARGET IS ALLOWED.** Ursa may aim Cosmic Omen at his own D20 Test, and with Read the Moment that means he can wait until his own die lands and then bend it. This supersedes the old others-only safe reading.
...
- **CONCENTRATION**: this competes directly with Summon Fey (and with Moonbeam, Entangle, Spike Growth and Polymorph). A sim must never run the pack and a summon at the same time. The expected play is pack + Starry Form (Archer), since Starry Form is not concentration.
- **PREPARED LIST CHANGED (DM, 2026-08-24).** **SUMMON BEAST is OUT; WITHER AND BLOOM is IN** (Arcana Unleashed, text transcribed with provenance in `reference/expansions/wither_and_bloom.md`). His 2nd level is now **Wither and Bloom, Spike Growth, Lesser Restoration**. Reasoning: Conjure Animals owns his concentration all day, which makes every *other* useful 2nd-level druid spell uncastable while the pack is out; Wither and Bloom is INSTANTANEOUS, so it is the only real damage option that coexists with the pack. Summon Beast was the natural cut because it is strictly worse than Summon Fey and wants the same concentration, so it was never actually cast. Aid was considered and rejected: **Lilly already prepares it** and has spare slots. Barkskin was considered and measured worse (NET 116 vs 105, and it degrades under pressure while Wither and Bloom improves).
- **PAGE 1 OF THE SHEET** now shows the **Spectral Pack** block where the Bestial Spirit used to be, beside the Fey Spirit, with the one-at-a-time concentration warning and the reminder that the pack's slashing does nothing to the Underroot Spike.
