...
- **Search `assets/` before generating any reference.** Lickgloom had canonical art at `assets/monsters/lickgloom.png` since Session 1 and a duplicate was generated anyway, in the wrong colours. The art is spread across `monsters/`, `npcs/`, `characters/`, `items/` and `art_refs/`, so grep all of them by name first. The superseded duplicate is in `_triage/superseded_art/`.
...
- **Ghostbloom is a FOUR-LEGGED spirit-beast**, a chubby translucent teal quadruped with a flame-like bloom of leaves growing from her back. She is not a wisp, a jellyfish or a floating fairy. A vague prompt talked the generator out of her attached reference twice.
